...
export default {
    async fetch(request: Request, env: Env): Promise<Response> {
        ...
        // API パス以外は静的アセットに委譲 (SPA フォールバック含む)
        ...
        if (request.method === 'OPTIONS') {
            return corsResponse(null, 204, origin, selfOrigin);
        }

        // Same-origin restriction (自サイトからのリクエストのみ許可)
        if (!isAllowedOrigin(origin, selfOrigin)) {
            return corsResponse(json({ error: 'Forbidden' }), 403, origin, selfOrigin);
        }
