...
import { setGamepads, setRelayActive, setKeymap } from '../../store/gamepadSlice';
...
import { saveKeymap, defaultKeymap } from '../../db/settings';
...
export default function GamepadPanel() {
    ...
    useEffect(() => {
        const update = () => {
            ...
            dispatch(setGamepads(gps.map(gp => ({
                index: gp.index,
                id: gp.id,
    ...
    const handleToggleRelay = (gpIndex: number, controllerId: number) => {
        ...
        if (gp.relayActive) {
            ...
            const client = getClient(wsUrl.replace('ws://', 'http://').replace('wss://', 'https://'), controllerId);
    ...
    return (
        <div className="panel gamepad-panel">
            ...
            <div className="tab-bar">
                <button className={`tab-btn ${tab === 'gamepads' ? 'active' : ''}`} onClick={() => setTab('gamepads')}>
                    デバイス
                </button>
                <button className={`tab-btn ${tab === 'keymap' ? 'active' : ''}`} onClick={() => setTab('keymap')}>
                    キーマップ
                </button>
            </div>
    ...
    return (
        <div className={`gamepad-row ${gp.relayActive ? 'relay-active' : ''}`}>
            ...
            <div className="relay-config">
                ...
                >
                    {[0, 1, 2, 3].map(i => <option key={i} value={i}>Switch #{i}</option>)}
                </select>
                <button
