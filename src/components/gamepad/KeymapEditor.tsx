...
const GAMEPAD_BUTTON_NAMES: Record<number, string> = {
    ...
    12: '↑', 13: '↓', 14: '←', 15: '→',
    16: 'Home', 17: 'Screenshot',
};

const SWITCH_BUTTON_NAMES: Record<number, string> = {
    [1 << 0]: 'Y', [1 << 1]: 'X', [1 << 2]: 'B', [1 << 3]: 'A',
    [1 << 4]: 'SR (右)', [1 << 5]: 'SL (右)', [1 << 6]: 'R', [1 << 7]: 'ZR',
    [1 << 8]: '−', [1 << 9]: '+', [1 << 10]: 'RS', [1 << 11]: 'LS',
    [1 << 12]: 'Home', [1 << 13]: 'SS',
    [1 << 16]: '↓', [1 << 17]: '↑', [1 << 18]: '→', [1 << 19]: '←',
    [1 << 20]: 'SR (左)', [1 << 21]: 'SL (左)', [1 << 22]: 'L', [1 << 23]: 'ZL',
};
...
export default function KeymapEditor({ keymap, onSave, onReset }: Props) {
    ...
    const handleSwitch = (gpBtn: number, switchBtn: number) => {
        setEditing(prev => prev.map(e =>
            e.gamepadButton === gpBtn ? { ...e, switchButton: switchBtn } : e,
        ));
    };
                ...
                <tbody>
                    {editing.map(entry => (
                        <tr key={entry.gamepadButton}>
                            <td>{GAMEPAD_BUTTON_NAMES[entry.gamepadButton] ?? `Button ${entry.gamepadButton}`}</td>
                            <td>→</td>
                            <td>
                                <select
                                ...
                                >
                                    {Object.entries(SWITCH_BUTTON_NAMES).map(([bit, name]) => (
                                        <option key={bit} value={Number(bit)}>{name}</option>
                                    ))}
                                </select>
                            </td>
                        </tr>
