...
function GuestCard({ guest, isPending, onAllow, onReject, onRemove, onSetController }: {
    guest: GuestStatus;
    isPending: boolean;
    ...
    const stateColor = {
        new: '#888', connecting: '#f90', connected: '#0a0',
        disconnected: '#666', failed: '#d00', closed: '#666',
    ...
    return (
        <div className={`guest-card ${isPending ? 'pending' : ''}`}>
            ...
            <div className="guest-info">
                ...
                <span className="guest-id" title={guest.userId}>{guest.userId.slice(0, 12)}…</span>
                {!isPending && (
            ...
            <div className="guest-actions">
                {isPending ? (
                    <>
                        <button className="btn btn-primary btn-sm" onClick={() => onAllow?.(selectedCid)}>
                            許可
                        </button>
...
export default function GuestList({ pending, guests, onAllow, onReject, onRemove, onSetController }: Props) {
    return (
