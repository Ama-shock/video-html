...
import { loadIdentity, saveIdentity, type StoredIdentity } from '../db/identity';
 ...
 */
export async function signMessage(identity: StoredIdentity, message: Uint8Array): Promise<Uint8Array> {
    const sig = await crypto.subtle.sign({ name: 'Ed25519' }, identity.privateKey, message);
